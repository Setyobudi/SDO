...
class WC_Appointments_Integration_GCal {
	...
	public function __construct() {
		...
		// Schedule incremental sync each minute.
		if ( ! as_next_scheduled_action( 'wc-appointment-sync-from-gcal' ) ) {
			$poll_interval = 1; // minutes.
			as_schedule_recurring_action( time(), $poll_interval * MINUTE_IN_SECONDS, 'wc-appointment-sync-from-gcal', array(), 'wca' );
		}
	...
	public function sync_from_gcal( $user_id = '' ) {
		...
		// Delete old actions from the AS log.
		// It logs every minute so it can get too big.
		if ( function_exists( 'as_delete_all_complete_actions' ) ) {
			as_delete_all_complete_actions( 'wc-appointment-sync-from-gcal', array(), 'wca' );
		}
	...
	public function gcal_availability_rules( $response ) {
		...
		foreach ( $response_data['items'] as $event ) {
			...
			// No avaailabilities, check if any appointment matches the event.
