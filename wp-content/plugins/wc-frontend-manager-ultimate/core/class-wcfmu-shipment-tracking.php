      ...
      if( $tracking_code && $tracking_url ) {
				if( wcfm_is_vendor() ) {
					...
					foreach( $product_ids as $product_id ) {
						$wcfm_messages = apply_filters( 'wcfm_shipment_tracking_message', sprintf( __( 'Vendor <b>%s</b> has shipped <b>%s</b> to customer.<br/>Tracking Code : %s <br/>Tracking URL : <a class="wcfm_dashboard_item_title" target="_blank" href="%s">%s</a>', 'wc-frontend-manager-ultimate' ), $shop_name, get_the_title( $product_id ), $tracking_code, $tracking_url, $tracking_url ), $tracking_code, $tracking_url, $product_id, $user_id );
						$WCFM->wcfm_notification->wcfm_send_direct_message( $user_id, 0, 0, 1, $wcfm_messages, 'shipment_tracking' );
					...
					foreach( $product_ids as $product_id ) {
						$comment_id = $order->add_order_note( apply_filters( 'wcfm_shipment_tracking_message', sprintf( __( 'Product <b>%s</b> has been shipped to customer.<br/>Tracking Code : %s <br/>Tracking URL : <a href="%s">%s</a>', 'wc-frontend-manager-ultimate' ), get_the_title( $product_id ), $tracking_code, $tracking_url, $tracking_url ), $tracking_code, $tracking_url, $product_id, $user_id ), apply_filters( 'wcfm_is_allow_shipment_note_to_customer', '1' ) );
					}
				}
			}
